	...
	override public function create()
	{
		...
		switch(SONG.stage)
		{
			...
			case 'halloweenbg':
				{
						defaultCamZoom = 0.6;
						curStage = 'halloweenbg';
						
					    var nightsky:FlxSprite = new FlxSprite(-970, -270).loadGraphic(Paths.image('nightsky'));
					    nightsky.scrollFactor.set(1.1, 1.1);
					    nightsky.scale.set(1.2, 1.2); 
					    add(nightsky);
					
					    var bg:FlxSprite = new FlxSprite(-730, -400).loadGraphic(Paths.image('bg'));
					    ...
					    var house:FlxSprite = new FlxSprite(-710, -410).loadGraphic(Paths.image('house'));
					    house.scrollFactor.set(0.9, 0.9);
					    house.scale.set(1.1, 1.1); 
					    add(house); 
					
					    var sidewalk:FlxSprite = new FlxSprite(-600, -540).loadGraphic(Paths.image('sidewalk'));
					    sidewalk.scrollFactor.set(1.0, 1.0);
					    sidewalk.scale.set(1.2, 1.2); 
					    add(sidewalk);  
		...
		if (curSong.toLowerCase() == 'timorous')
			{
				switch(curStep)
				{
					...
					case 64:
					    FlxTween.tween(FlxG.camera, {zoom: 0.6}, 1, {ease: FlxEase.quadInOut});
						new FlxTimer().start(1 , function(tmr:FlxTimer)
						{
							defaultCamZoom = 0.6;
						});
